...
{
public:
	//ke bian shu ru can shu
	string pcd_file_path;
	string yaml_file_path;
	string name_of_pcd;
	string nbv_net_path;

	int num_of_views;					//yici caiyang shidian geshu
	double cost_weight;
	rs2_intrinsics color_intrinsics;
	double depth_scale;

	//yunxing canshu
	int process_cnt;					//guocheng bianhao
	atomic<double> pre_clock;			//xitongshizhong   atomic quebaoduoxianchengzhongbuhuichuxianshujujingzheng
	atomic<bool> over;					//guochengshifoujieshu
	bool show;
	int num_of_max_iteration;

	//dianyun shuju
	atomic<int> vaild_clouds;
	vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr > clouds;							//dianyun zu
	...
	double predicted_size;						//wuti BBX banjing
	...
	Share_Data(string _config_file_path, string test_name = "", int _sampling_method = -1,int _need_case_1 = -1)
	{
		...
		//duqu zhuanhuanhoude1 pcd moxing wenjian
		...
		//xuanzhuan conger shi Z zhou chaoshang
		...
		//tiaozheng daxiao baozheng shuimi biaomian
