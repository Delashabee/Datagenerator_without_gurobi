...
inline double get_random_coordinate(double from, double to) {
	//shengcheng bijiao suijide1 suijishu 0dao 1 yingshe dao qujian[from,to]
	double len = to - from;
...
{
public:
	Eigen::Vector3d init_pos;	//chu shi weizhi 
	Eigen::Matrix4d pose;		//view_i dao view_i+1 de1xuanzhuanjuzhen
	...
	void get_next_camera_pos(Eigen::Matrix4d now_camera_pose_world, Eigen::Vector3d object_center_world) {
		...
		//dingyi zhixiangwutiwei Z+ congshangyigexiangjiweizhifachushexian1dao1dangqianwei X+ jisuanliangge1zuobiaoxizhijiande1bianhuanjuzhen 
		// object and view is the coordinate from the last camera coordinate system
		...
		//rao z zhou xuanzhuan shide1yushangyicixuanzhuanjisuande1 X yu Y de1 jiajiao zuixiao
...
{
public:
	int num_of_views;							//shidian geshu
	vector<View> views;							//kongjiande caiyangshidian
	Eigen::Vector3d object_center_world;		//wuti zhongxin
	double predicted_size;						//wutide BBX banjing
	Eigen::Matrix4d now_camera_pose_world;		//zheci nbv diedai de xiangji weizhi
	int occupied_voxels;						
	double map_entropy;	
